...
```{r eval = FALSE}
# we cannot see the shapes of all of them, and it's not good for Red-green colour-blindness
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r eval = FALSE}
# What does scale_fill_manual do?to change the color of the density plot
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r eval = FALSE}
# What does alpha = 0.5 do? change
# Play around with different values of alpha, between 0 and 1
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density(alpha = 0.35, color = "white") +
  labs(x = "Republican support", fill = "historical voting trends")+
  scale_fill_manual(values = c("blue", "purple", "red"))+
  theme_minimal()
```

```{r eval = FALSE}
# What does facet_wrap do?! separate each of them and make each of them a plot.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
-   Which do you like best? the transparent one
-   What is one pro of density plots relative to boxplots? it can show the difference more obviously 
-   What is one con of density plots relative to boxplots? tell us the mean, outliers and ect.
...
```{r eval = FALSE}
# A stacked bar plot
# How are the "historical" and "winner_20" variables mapped to the plot, i.e. what roles do they play? The historical is the x-axis in the plot, and the winner_20 counts the dem and repub that won in each historical.
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
...
> We can see the proportion of each winner, but cannot the the exact count of each of them
