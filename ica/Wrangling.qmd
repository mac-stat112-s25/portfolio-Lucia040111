...
```{r}
# What did this code do? Since repub_pct_20 > dem_pct_20 gives use true or false results, we ad a new column 
#repub_win_20 that all the cells are true or false
elections_small |>
...
elections_small |>
 mutate(diff_dem = dem_pct_16 - dem_pct_20) |>
 head()
```
...
elections_small |>
 mutate(dem_20_16 = dem_pct_20>dem_pct_16) |>
 head()
```
...
> It won't work, because we select column that doesn't exist
...
elections_small |>
  mutate(total_dem = round(total_votes_20 * dem_pct_20 / 100), total_rep =  round(total_votes_20 * repub_pct_20 / 100))%>%
  group_by(state_name)%>%
  summarize(total_dem = median(total_dem), total_rep = sum(total_rep))
