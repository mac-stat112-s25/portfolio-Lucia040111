...
penguins %>% filter(species == 'Adelie', body_mass_g>4700)
...
penguins %>% arrange(desc(body_mass_g)) %>% head(6)
...
penguins %>% group_by(species)%>%summarize(avg = mean(body_mass_g, na.rm = TRUE))

# Create a new column that records body mass in kilograms, not grams
# NOTE: there are 1000 g in 1 kg
# Show just the first 6 rows

penguins %>% mutate(body_mass_kg = body_mass_g/1000)%>%head()
```
...
daily_births <- Birthdays %>%
  group_by(date)%>%
  summarise(total_birth = sum(births))%>%
...
```{r}
daily_births %>%
  ggplot(aes(x = week_day, y = total_birth))+
  geom_boxplot()
```
...
```{r}
Birthdays %>%
  filter(year == 1979, month == 9, day <= 12 & day >=9)%>%
  group_by(state) %>%
  summarize(total_births = sum(births), .groups = "drop") %>%
