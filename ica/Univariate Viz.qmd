...
```{r}
# There are bars adding into the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```

```{r}
# there are new x labels and y label
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# The color of bars changes to blue
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# The color of frame changes to orange
ggplot(hikes, aes(x = rating)) +
...
```{r}
# The change of theme make the background gray to white
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
-   What's the purpose of the `+`?
    When do we use it?
    `+` means that we want to add new things to the plot.
...
-   What's the difference between `color` and `fill`?
    `color` is the color of frame and `fill` choose the color that fill in the bar.

-   Why does adding `color = "white"` improve the visualization?
    \`it makes the bars easy to look at

-   What did `binwidth` do?
    `binwidth` put elevation equaly into groups so that there are less bars and each of them is wider.
